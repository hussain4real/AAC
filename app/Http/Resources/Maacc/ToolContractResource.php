 ...
 * (resources/js/maacc/data.ts `Tool`). `execMode` and `impl` keep their raw
...
class ToolContractResource extends JsonResource
{
    ...
    private function knowledgeConfigView(): ?array
    {
        ...
        return [
            'topK' => (int) ($config['top_k'] ?? config('maacc.runtime.knowledge.default_top_k', 5)),
    ...
    private function dbConfigView(): ?array
    {
        ...
        return [
            ...
            'rowLimit' => (int) ($config['row_limit'] ?? config('maacc.runtime.db.default_row_limit', 50)),
