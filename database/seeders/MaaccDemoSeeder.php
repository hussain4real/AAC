 ...
 * (resources/js/maacc/data.ts) as governed database records. Every entity keeps
 ...
 */
class MaaccDemoSeeder extends Seeder
{
    use WithoutModelEvents;

    /**
     * Seed the MAACC platform data for the demo team.
     */
    ...
    private function seedEnterprise(Team $team, User $user, array $apps, array $projects, array $agents, array $llms): void
    {
        ...
        SsoConnection::updateOrCreate(['slug' => 'milaha-entra-id'], [
            ...
            'client_id' => 'maacc-console-app',
            'client_secret' => 'demo-entra-secret',
            'scopes' => 'openid profile email groups',
            'default_team_role' => TeamRole::Member,
            'group_role_mappings' => [
                ['group' => 'MAACC-Platform-Admins', 'team_role' => 'admin'],
                ['group' => 'MAACC-Developers', 'team_role' => 'member', 'maacc_role' => 'developer', 'project_slug' => array_values($projects)[0]->slug],
            ],
            'auto_provision' => true,
    ...
    private function seedToolContracts(Team $team, array $apps): array
    {
        $rows = [
            ...
            ['summarizeUploadedDocument', 'global', 'hosted', 'internal', false, 'ready', null, 'Summarizes an uploaded document passed inline to MAACC.', 20, 1024, ['document_ref' => 'string', 'length' => 'string?'], ['summary' => 'string', 'key_points' => 'array']],
    ...
    /**
     * Assign the demo user MAACC project roles for a few projects so the console
    ...
    {
        $assignments = [
            'prj_mop_ops' => MaaccRole::ProjectOwner,
            'prj_fws_appr' => MaaccRole::Developer,
            'prj_csp_trend' => MaaccRole::Viewer,
        ];

        foreach ($assignments as $slug => $role) {
            $projects[$slug]->members()->syncWithoutDetaching([
                $user->id => ['maacc_role' => $role->value],
            ]);
        }
    }
     ...
     * string or credential is persisted by MAACC — only the approved connection
     * name reference.
     *
     * @param  array<string, ToolContract>  $tools
     */
    ...
    private function buildReportingReplica(): void
    {
        ...
        $database = config('database.connections.maacc_reporting.database');

        if (is_string($database) && $database !== ':memory:' && ! file_exists($database)) {
            touch($database);
        }

        $schema = Schema::connection('maacc_reporting');
        ...
        DB::connection('maacc_reporting')->table('reporting_port_calls')->insert([
