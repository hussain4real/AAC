# MAACC Platform Administration

> MAACC's own internal administration model (Phase 8B): a dedicated, global platform-administration RBAC layer backed by `spatie/laravel-permission`. Audience: MAACC platform admins, security reviewers, and operators.

## The model in one paragraph

MAACC has **two** authorization layers. The **tenant RBAC** (team `Owner`/`Admin`/`Member` + project `MaaccRole`) governs a team's own application workflows and is unchanged. The **platform RBAC** described here is a *global* layer held by MAACC's own operators that governs the platform itself — managing other users' access, cross-tenant governance, audits, and operational sign-off. **A user with no platform role is a pure tenant user** and can never reach the platform-admin surfaces. The two are independent: holding a platform role does not change a user's tenant workflows, and tenant roles never confer platform administration.
...
- **Bootstrap Super Admins** by email so the platform is never left without an administrator: `MAACC_PLATFORM_SUPER_ADMINS="ops@milaha.com,security@milaha.com"`. Empty by default in production — assign through the console or SSO instead.
...
For incidents, a Super Admin can grant **time-boxed** emergency platform access. The TTL is clamped to `config('maacc.platform.break_glass')` (`default_ttl_minutes`, `max_ttl_minutes`). The grant auto-expires; the scheduled review (below) revokes any elapsed break-glass grant and removes the role.

## Access review

`maacc:review-platform-access` (scheduled daily) is the periodic certification + cleanup:

```bash
php artisan maacc:review-platform-access          # revoke expired break-glass; report review work
php artisan maacc:review-platform-access --json   # machine-readable
```
...
`config('maacc.platform')`:

| Key | Purpose | Default |
| --- | --- | --- |
| `super_admins` | Bootstrap Super Admin emails (`MAACC_PLATFORM_SUPER_ADMINS`) | `[]` |
...
- Run `maacc:review-platform-access` on a schedule (it is registered in `routes/console.php`) so break-glass never lingers.
- Tenant data boundaries are unchanged: a platform role grants MAACC administration, not direct access to a tenant application's data — application-owned tools still run in the owning application.
