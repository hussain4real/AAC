...
Milaha AI Agent Center (MAACC) is a proposed centralized AI agent orchestration platform for the company. The platform will allow internal application teams to create, configure, govern, and expose AI agents through a secure web interface and API endpoints.

MAACC will provide a consistent way for company applications to use approved Large Language Models (LLMs), define project-specific agents, attach governed tools, integrate through SDKs, and monitor agent activity through auditable logs and usage reports.

A key business principle is that MAACC should orchestrate agent behavior and tool contracts while application teams retain ownership of application-specific data access, permissions, and business logic. Application-specific tools will be defined first in the MAACC UI as contracts, then implemented locally by the owning application through the MAACC SDK.
...
MAACC is intended to provide a reusable enterprise platform where internal teams can create and manage AI agents in a secure, governed, auditable, and scalable manner.

---

## 3. Business Objectives

The objectives of MAACC are to:
...
6. Allow application-specific tools to be implemented within the owning application using the MAACC SDK.
...
The initial scope of MAACC shall include:
...
- Direct unrestricted database access from MAACC to application production databases.
...
|---|---|
| MAACC Platform Owner | Owns platform roadmap, governance, and operating model. |
...
| IT / Infrastructure Team | Hosts, monitors, and operates MAACC and related services. |
...
| Term | Definition |
|---|---|
| MAACC | Milaha AI Agent Center, the central AI agent orchestration platform. |
| Project | A logical container in MAACC, typically mapped to an internal application, business domain, or delivery initiative. |
| Application | A company system that integrates with MAACC to consume agents or implement tools. |
...
| Tool Contract | The MAACC definition of a tool, including name, description, input schema, output schema, execution mode, and governance metadata. |
...
| Client-Side Tool | A tool whose execution is delegated to the calling application through the MAACC SDK. |
...
| MAACC SDK | A software package installed in company applications to authenticate, discover agents, invoke agents, register local tool handlers, and handle pause-and-resume execution. |
...
MAACC will consist of a central platform and integration capabilities for company applications.
...
A company application will integrate with MAACC by installing the MAACC SDK and configuring credentials generated from the MAACC platform.
...
> Application developers create tool contracts first in the MAACC UI, then implement those tools inside the owning application using the MAACC SDK.

This means:

- MAACC owns the tool definition and governance metadata.
- The owning application owns the tool execution logic.
- MAACC does not need direct access to application databases for client-side tools.
- Application developers can see which tools require local implementation.
- The SDK provides the bridge between MAACC and the application.
...
1. A MAACC admin or authorized project owner registers a project or application in MAACC.
2. MAACC creates a project record.
3. MAACC generates environment-specific credentials.
4. Application developers install the MAACC SDK in the application.
5. Application developers configure SDK credentials securely in the application environment.
6. The application connects to MAACC and can discover available agents and required tool implementations.

### 9.2 Agent Creation Journey

1. A developer opens the relevant project in MAACC.
...
7. MAACC marks client-side tools as requiring implementation in the owning application.
...
1. Application developers view the required tool implementation checklist in MAACC.
2. MAACC displays missing, implemented, outdated, or incompatible client-side tools.
3. MAACC displays or generates SDK implementation stubs.
4. Developers implement the required tool handlers inside the application.
5. The application SDK reports implemented tools back to MAACC.
6. MAACC validates the implementation status against the tool contract.
...
2. The application invokes a MAACC agent through the SDK or API.
3. MAACC starts an agent run.
4. The agent determines whether a tool is required.
5. If a client-side tool is required, MAACC pauses execution and returns a tool request to the calling application.
...
8. The application returns the tool result to MAACC.
9. MAACC validates the result and resumes the agent run.
10. MAACC returns the final response to the calling application.
...
The Client-Side Tool Execution Pattern allows MAACC agents to use application-specific capabilities without MAACC directly accessing application databases or internal services.

This pattern is useful when:

- Applications must keep databases isolated.
- MAACC should not hold credentials for application databases.
...
```text
Application invokes MAACC agent
        ->
MAACC runs the agent
        ->
Agent requires a client-side tool
        ->
MAACC returns a tool request to the application SDK
        ->
SDK executes the matching local tool handler
        ->
Application accesses its own data or services
        ->
Application returns tool result to MAACC
        ->
MAACC resumes the agent run
        ->
MAACC returns final response
```
...
```typescript
maacc.registerTool("businessDataLookup", async (args, context) => {
...
MAACC should support multiple tool execution modes so that each use case can use the safest and most practical integration pattern.

| Execution Mode | Description | Typical Use |
|---|---|---|
| MAACC-hosted | Tool runs inside MAACC. | Web search, generic utilities, shared tools. |
...
| Remote HTTP | MAACC calls an approved internal API. | Existing internal services exposed through APIs. |
| Connector server | MAACC calls an application-owned connector service. | Advanced integrations and long-running connectors. |
| Knowledge retrieval | Tool searches indexed documents or knowledge bases. | Policies, manuals, contracts, FAQs. |
| Read-only database | MAACC reads from approved views or replicas under strict control. | Controlled analytics and reporting use cases. |

For the initial release, the recommended priority is:

1. MAACC-hosted tools.
2. Client-side tools.
3. Remote HTTP tools.
...
|---|---|---|
| FR-001 | MAACC shall allow authorized users to register an application or project. | Must Have |
| FR-002 | MAACC shall generate secure credentials for each registered application environment. | Must Have |
| FR-003 | MAACC shall support separate environments such as development, staging, and production. | Should Have |
| FR-004 | MAACC shall allow project owners to manage project members and permissions. | Must Have |
| FR-005 | MAACC shall allow admins to disable or revoke application credentials. | Must Have |

### 12.2 Agent Management

| ID | Requirement | Priority |
|---|---|---|
| FR-006 | MAACC shall allow developers to create agents under a project. | Must Have |
| FR-007 | MAACC shall require a system prompt or use-case description when creating an agent. | Must Have |
| FR-008 | MAACC shall allow developers to select an LLM from a company-approved model list. | Must Have |
| FR-009 | MAACC shall support draft and published states for agents. | Must Have |
| FR-010 | MAACC shall support versioning of agents. | Should Have |
| FR-011 | MAACC shall provide an agent playground for testing. | Should Have |
| FR-012 | MAACC shall expose a secure API endpoint for each published agent. | Must Have |

### 12.3 LLM Management

| ID | Requirement | Priority |
|---|---|---|
| FR-013 | MAACC shall maintain a catalog of company-approved LLMs. | Must Have |
| FR-014 | MAACC shall allow admins to enable or disable LLMs globally. | Must Have |
| FR-015 | MAACC shall allow admins to restrict LLM availability by project. | Should Have |
| FR-016 | MAACC shall track token usage and estimated cost per model. | Must Have |
| FR-017 | MAACC should support model routing policies in future releases. | Could Have |

### 12.4 Tool Management

| ID | Requirement | Priority |
|---|---|---|
| FR-018 | MAACC shall allow admins to create and manage global tools. | Must Have |
| FR-019 | MAACC shall allow project owners to create project-level tools. | Must Have |
| FR-020 | MAACC shall allow developers to create agent-level tools. | Must Have |
| FR-021 | MAACC shall require each tool to have a name, description, input schema, output schema, and execution mode. | Must Have |
| FR-022 | MAACC shall support client-side tool contracts created through the UI. | Must Have |
| FR-023 | MAACC shall display which client-side tools require implementation inside the owning application. | Must Have |
| FR-024 | MAACC shall validate tool input arguments against the tool input schema. | Must Have |
| FR-025 | MAACC shall validate tool results against the tool output schema. | Must Have |
| FR-026 | MAACC shall support tool approval before production use. | Should Have |

### 12.5 SDK Integration

| ID | Requirement | Priority |
|---|---|---|
| FR-027 | MAACC shall provide an SDK for applications to authenticate with MAACC. | Must Have |
| FR-028 | The SDK shall allow applications to list agents available under their project. | Must Have |
| FR-029 | The SDK shall allow applications to invoke MAACC agents. | Must Have |
...
| FR-033 | The SDK shall report implemented tools back to MAACC. | Must Have |
...
| ID | Requirement | Priority |
|---|---|---|
| FR-035 | MAACC shall create an agent run for each invocation. | Must Have |
| FR-036 | MAACC shall support run statuses including queued, running, requires_tool, completed, failed, expired, and cancelled. | Must Have |
| FR-037 | MAACC shall pause an agent run when client-side tool execution is required. | Must Have |
| FR-038 | MAACC shall return tool execution requests to the calling application. | Must Have |
| FR-039 | MAACC shall resume an agent run after receiving tool results. | Must Have |
| FR-040 | MAACC shall support timeouts for pending tool execution. | Must Have |
| FR-041 | MAACC shall support synchronous agent calls. | Must Have |
| FR-042 | MAACC should support asynchronous and streaming agent calls. | Should Have |

### 12.7 Observability and Audit

| ID | Requirement | Priority |
|---|---|---|
| FR-043 | MAACC shall log each agent run. | Must Have |
| FR-044 | MAACC shall log model used, token usage, duration, and status. | Must Have |
| FR-045 | MAACC shall log tool calls and tool execution metadata. | Must Have |
| FR-046 | MAACC shall allow configuration of whether tool results are stored, masked, or excluded from logs. | Must Have |
| FR-047 | MAACC shall provide dashboards for usage and cost per project, agent, and model. | Should Have |
| FR-048 | MAACC shall allow admins to inspect failed runs. | Should Have |

### 12.8 Security and Governance

| ID | Requirement | Priority |
|---|---|---|
| FR-049 | MAACC shall support role-based access control. | Must Have |
| FR-050 | MAACC shall authenticate applications using generated credentials. | Must Have |
| FR-051 | MAACC shall support credential rotation. | Must Have |
| FR-052 | MAACC shall allow admins to revoke application access. | Must Have |
| FR-053 | MAACC shall pass caller context to client-side tools where applicable. | Must Have |
| FR-054 | MAACC shall enforce model and tool access policies. | Must Have |
| FR-055 | MAACC shall support data sensitivity classification for tools. | Should Have |
| FR-056 | MAACC shall support prompt and tool-call guardrails. | Should Have |
...
|---|---|
| Security | MAACC must not require direct access to application databases for client-side tools. |
| Availability | MAACC should be highly available for production agent endpoints. |
| Performance | Simple agent calls should return within acceptable application UX limits. |
| Scalability | MAACC should support multiple projects, agents, applications, and concurrent runs. |
...
| Extensibility | MAACC should support additional tool execution modes in future releases. |
...
MAACC distinguishes **tenant-scoped roles** (governing a team/application's own workflows) from **MAACC platform-administration roles** (governing the platform itself). A tenant user holds **no** platform role by default and cannot reach the platform-admin surfaces; platform administration, approvals, pilot readiness, release baselines, and operational sign-off are MAACC-admin responsibilities, not tenant-user workflows.
...
### 14.2 MAACC platform-administration roles (global)

Implemented as a dedicated, Spatie-backed global RBAC layer (see `docs/MAACC_Platform_Administration.md`), held by MAACC's own operators and layered above the tenant RBAC. The Super Admin holds an unrestricted authorization override; assigning Super Admin and activating break-glass are Super-Admin-only. Tenant users are mapped into these roles only by explicit grant or SSO group mapping.

| Platform Role | Remit |
|---|---|
| Super Admin | Unrestricted MAACC platform control, including break-glass and assigning any role. |
...
MAACC shall maintain a status for each client-side tool per application environment.

| Status | Meaning |
|---|---|
| Not Required | The tool is not used by any published agent in that environment. |
| Requires Implementation | The tool contract exists in MAACC but has no reported implementation in the application. |
...
| waiting_for_client | MAACC is waiting for the calling application to return a client-side tool result. |
...
1. MAACC shall not directly access application production databases for client-side tools.
...
6. MAACC shall validate tool result structure before passing it into the agent reasoning cycle.
7. MAACC shall allow policies to control whether raw tool results are stored in logs.
8. MAACC shall allow maximum payload size and timeout limits for tool results.
9. MAACC shall encourage summary or aggregate tool results where possible instead of large raw datasets.
...
When an application or project environment is registered in MAACC, MAACC shall generate credentials such as:

```env
MAACC_PROJECT_ID=project_environment_id
MAACC_CLIENT_ID=client_id
MAACC_CLIENT_SECRET=client_secret
MAACC_ENVIRONMENT=production
```

The credentials shall allow the application to authenticate with MAACC and access only the agents and tools assigned to the relevant project and environment.

### 18.2 SDK Usage Pattern

```typescript
const maacc = new MAACClient({
...
const response = await maacc.runAgent("agentIdentifier", {
...
MAACC should provide reports and dashboards for:
...
|---|---|---|
| Tool implementations differ from MAACC contracts. | Agent failure or incorrect outputs. | Schema validation, versioning, SDK status sync. |
...
| MAACC becomes a central dependency. | Platform outage impacts integrations. | High availability, graceful degradation, retries. |

---

## 21. Assumptions

1. MAACC will be deployed as a central internal platform.
2. Company applications can install and configure a MAACC SDK.
...
5. MAACC will authenticate applications using generated credentials.
...
8. Tool contracts will be created first in MAACC UI before local implementation.
...
1. What technology stack will be used for the MAACC platform?
2. Which programming languages should the first SDKs support?
3. Should MAACC support both REST and streaming APIs in the first release?
...
6. How should MAACC handle long-running tool calls?
...
9. Should agents be available only through integrated applications or also through a MAACC chat UI?
10. What identity provider will MAACC use for user authentication?
11. Should end-user identity be passed from the calling application to MAACC and back to client-side tool handlers?
...
6. Client-side tool contract creation in MAACC UI.
...
```text
Register project in MAACC
        ->
Create agent
        ->
Create required tool contract in MAACC UI
        ->
Install MAACC SDK in application
        ->
Implement local tool handler
        ->
Invoke agent from application
        ->
MAACC requests client-side tool execution
        ->
Application executes tool locally
        ->
MAACC resumes run and returns final response
```
...
MAACC will be considered successful when:

1. Application teams can create agents without building separate AI infrastructure.
2. Applications can securely call MAACC agents through SDK or API.
3. Client-side tools can access application-owned data without exposing database credentials to MAACC.
...
MAACC should be positioned as a central enterprise AI agent orchestration platform supported by application SDKs that allow internal systems to securely connect to agents and implement application-owned tools.

The strongest architectural choice for application-specific data access is the client-side tool execution pattern. This allows MAACC to orchestrate agents and define tool contracts while leaving database access, permissions, and business logic inside the owning application.
