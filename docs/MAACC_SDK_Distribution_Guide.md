# MAACC SDK Distribution Guide

This guide describes the private, manual-first release process for making the
MAACC SDKs available to pilot application teams.

## Package Names

| Language | Package | Version | Distribution |
|----------|---------|---------|--------------|
| PHP | `maacc/sdk` | `0.2.0` | Private Composer VCS package repository |
| TypeScript | `@qatar-navigation-milaha/sdk` | `0.2.0` | GitHub Packages npm registry |

The MAACC API contract remains `v0.0.1`. These package versions are client
...
registry. For the manual pilot release, PHP consumers install `maacc/sdk` from a
private Composer-compatible source: either a split PHP SDK repository whose root
`composer.json` is `maacc/sdk`, or a package/artifact repository such as Private
Packagist. Do not point Composer VCS at the MAACC monorepo root; Composer reads
the root `composer.json` and will not discover the nested
`packages/maacc-sdk-php/composer.json` package.
...
root package is the tagged `maacc/sdk` package:
...
```json
{
  "repositories": [
    {
      "type": "vcs",
      "url": "https://github.com/OWNER/maacc-sdk-php.git"
    }
  ],
  "require": {
    "maacc/sdk": "^0.2"
...
use the path repository that points at `../../packages/maacc-sdk-php`.
...
| Variable | Purpose |
|----------|---------|
| `MAACC_BASE_URL` | Base URL for the MAACC instance. |
| `MAACC_CLIENT_ID` | Application credential client id. |
| `MAACC_CLIENT_SECRET` | Application credential secret shown once on generate/rotate. |
| `MAACC_AGENT_SLUG` | Published agent slug used by the app or reference consumer. |
| Tool slug mappings | App-specific env vars that map local handlers to MAACC tool contracts. |
...
```bash
composer validate packages/maacc-sdk-php/composer.json
(cd packages/maacc-sdk-ts && npm run pack:dry)
npm run types:check:sdk
npm run test:sdk
composer test:reference
php artisan maacc:sdk-fixtures --check
composer ci:check
...
2. Confirm the SDK READMEs and integration docs use `maacc/sdk` and `@qatar-navigation-milaha/sdk`.
...
Composer registry configured, install `maacc/sdk:^0.2`, and verify autoloading:

```bash
php -r "require 'vendor/autoload.php'; echo Maacc\\Sdk\\MaaccClient::VERSION.PHP_EOL;"
```
