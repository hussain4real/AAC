...
Milaha AI Agent Center (MAACC) is a centralized enterprise AI agent orchestration platform. It enables internal application teams to register applications, create projects, configure AI agents, select company-approved Large Language Models (LLMs), define governed tools, expose agents through secure APIs, and monitor agent usage through audit logs and operational dashboards.

The key architectural principle is that MAACC orchestrates AI behavior and governs tool contracts, while application teams retain ownership of application-specific data access, permissions, and business logic.

MAACC supports multiple tool execution modes, with special emphasis on the **Client-Side Tool Execution Pattern**, where application-specific tools are defined in MAACC first and then implemented inside the integrating application through the MAACC SDK. This allows agents to use application data without MAACC directly accessing application databases.
...
|---|---|
| Centralized governance, decentralized execution | MAACC centrally governs agents, models, tool contracts, and audit logs, while applications execute their own local business tools. |
| Application data remains application-owned | MAACC should not require unrestricted direct access to application databases. |
| Tool contracts before implementations | Developers define tool contracts in MAACC UI before implementing them through the SDK. |
...
| Extensible runtime | MAACC should support multiple execution modes, LLM providers, tools, and integration patterns. |
...
MAACC sits between company applications, approved LLM providers, governed tools, enterprise identity, and platform observability services.

```mermaid
flowchart LR
    Users[Business Users / Developers / Admins]
    Apps[Registered Company Applications]
    SDK[MAACC SDK inside Applications]
    MAACC[MAACC Platform]
...
| Actor | Interaction with MAACC |
|---|---|
| MAACC Admin | Configures platform-level settings, approved LLMs, global tools, governance, and roles. |
...
| Registered Application | Calls MAACC agents through API/SDK and executes client-side tools when required. |
...
MAACC is composed of six major layers:
...
The MAACC Web UI provides administrative, developer, governance, and operational screens.
...
The Client-Side Tool Bridge enables MAACC to request tool execution from the calling application.
...
The MAACC SDK is installed inside registered company applications.

Responsibilities:

- Authenticate application with MAACC
- Discover available agents
- Invoke MAACC agents
- Fetch required client-side tool contracts
- Register local tool handlers
- Execute local handlers when MAACC requests a tool
- Submit tool results back to MAACC
...
| Entity | Purpose |
|---|---|
| Application | A company system registered to integrate with MAACC. |
...
| Role / Permission | Access control structure for MAACC users. |
...
MAACC supports three tool scopes:
...
|---|---|---|
| MAACC-hosted | Tool runs inside MAACC platform. | MAACC-owned execution. |
| Client-side | Tool is executed by the calling application through the SDK. | Application-owned execution. |
| Remote HTTP | MAACC calls an approved internal service endpoint. | Service-owned execution. |
| Connector server | MAACC calls a managed connector service owned by the application/team. | Connector-owned execution. |
| Knowledge retrieval | MAACC retrieves from approved indexed documents or knowledge stores. | MAACC or enterprise knowledge layer. |
| Read-only database view | MAACC reads from approved restricted views, never unrestricted production tables. | Controlled exception. |
...
Client-side tool execution allows MAACC agents to request application-specific data or business actions without MAACC directly connecting to application databases.

In this model:

- MAACC defines the tool contract.
- The application implements the tool logic locally.
- The SDK handles communication between the application and MAACC.
- The application enforces local permissions, business rules, and data filtering.
- MAACC validates the tool result shape before resuming the agent run.
...
sequenceDiagram
    ...
    participant SDK as MAACC SDK
    participant MAACC as MAACC Runtime
    ...
    SDK->>MAACC: Create agent run
    MAACC->>MAACC: Load agent config and tools
    MAACC->>LLM: Send prompt and tool definitions
    LLM-->>MAACC: Tool call requested
    MAACC->>MAACC: Set run status = requires_tool
    MAACC-->>SDK: Tool execution required
    ...
    SDK->>MAACC: Submit tool result
    MAACC->>MAACC: Validate result and resume run
    MAACC->>LLM: Continue reasoning with tool result
    LLM-->>MAACC: Final response
    MAACC-->>SDK: Completed response
...
```typescript
maacc.registerTool("getBusinessRecords", async (args, context) => {
  ...
  // 5. Return result matching MAACC output schema
});
```
...
```php
$maacc->registerTool('getBusinessRecords', function (array $args, array $context) {
    // Validate arguments
...
MAACC should track implementation status per application and environment.
...
- MAACC displays compatibility status.
...
flowchart TD
    ...
    J -- MAACC-hosted --> K[Execute Inside MAACC]
...
The SDK provides a standard integration bridge between registered applications and MAACC.

Responsibilities:

1. Authenticate the application using MAACC-issued credentials.
...
7. Submit tool results to MAACC.
8. Support retries, timeouts, and error reporting.
9. Report implemented tool versions to MAACC.
...
flowchart TB
    subgraph App[Registered Application]
        ...
        subgraph SDK[MAACC SDK]
            SDKClient[MAACC API Client]
    ...
    SDKClient <--> MAACC[MAACC Platform]
```
...
| Polling mode | Application polls MAACC for run state changes. | Systems with limited streaming support. |
| Webhook mode | MAACC notifies application when action is required. | Long-running or asynchronous workflows. |
...
|---|---|
| Management API | Used by MAACC UI and admins to manage applications, projects, agents, tools, and models. |
...
flowchart LR
    ...
    subgraph MAACCBoundary[MAACC Security Boundary]
        MAACC[MAACC Runtime]
...
- Enterprise SSO for MAACC Web UI users.
...
| Level | Control |
|---|---|
| Platform | Who can administer MAACC settings. |
...
- Application must authenticate to MAACC.
- MAACC must validate that the tool is assigned to the requesting agent.
- MAACC must validate tool arguments against input schema.
...
- MAACC must validate result size and structure before resuming.
...
flowchart TB
    ...
    subgraph MAACCluster[MAACC Application Layer]
        WebApp[MAACC Web App]
...
|---|---|
| Web Application | MAACC UI and management APIs. |
...
```mermaid
sequenceDiagram
    participant Admin as MAACC Admin / Project Owner
    participant UI as MAACC UI
    participant MAACC as MAACC Backend
    participant Vault as Secrets Vault
    participant App as Registered Application

    Admin->>UI: Register application
    UI->>MAACC: Submit application metadata
    MAACC->>MAACC: Create application record
    MAACC->>Vault: Generate/store client secret
    MAACC-->>UI: Return client ID and setup details
    Admin->>App: Configure MAACC SDK credentials
    App->>MAACC: Authenticate and sync manifest
    MAACC-->>App: Return available agents and required tools
```
...
sequenceDiagram
    ...
    Dev->>UI: Create agent
    UI->>MAACC: Submit prompt, model, tools, settings
    MAACC->>Policy: Validate model and tool permissions
    Policy-->>MAACC: Approved or rejected
    MAACC->>MAACC: Save draft agent
    Dev->>UI: Test agent in playground
    UI->>MAACC: Start test run
    MAACC-->>UI: Return test result and trace
    Dev->>UI: Publish agent
    UI->>MAACC: Publish request
    MAACC->>Policy: Apply publication rules
    MAACC-->>UI: Agent published
```
...
sequenceDiagram
    ...
    participant UI as MAACC UI
    participant MAACC as Tool Registry
    participant App as Application SDK

    Dev->>UI: Create tool contract
    UI->>MAACC: Submit name, schema, scope, execution mode
    MAACC->>MAACC: Validate schema
    MAACC->>MAACC: Save tool contract
    MAACC-->>UI: Tool created
    App->>MAACC: Sync required tools
    MAACC-->>App: Return tool contract and version
    App->>App: Developer implements local handler
    App->>MAACC: Report implementation status
```
...
Before an agent is published, MAACC should verify:
...
|---|---|
| Availability | MAACC should be designed for high availability in production. |
...
|---|---|---|
| AD-001 | MAACC is a central platform, not only an SDK. | Central governance, visibility, agent management, and auditability are required. |
...
| AD-003 | Tool contracts are created first in MAACC UI. | Provides central governance and visibility before implementation. |
| AD-004 | Client-side tools are implemented in the owning application. | Preserves application data ownership and reduces MAACC database access risk. |
| AD-005 | MAACC should not use unrestricted direct production DB access as the default pattern. | Avoids security, coupling, and governance risks. |
...
1. Which identity provider will be used for MAACC Web UI authentication?
2. Which LLM providers and deployment models are approved for the first release?
3. Which programming languages should the MAACC SDK support first?
...
9. Should MAACC support direct read-only database views in the first release or defer this to a later phase?
10. Which monitoring, SIEM, or log aggregation tools should MAACC integrate with?
...
The proposed MAACC architecture provides a strong enterprise foundation for safe, governed, and reusable AI agent adoption. The design separates orchestration from application-owned execution, allowing MAACC to manage agents, prompts, tools, LLMs, governance, and auditability while preserving application-level control over data access and business rules.

The Client-Side Tool Execution Pattern is the most important design choice for integrating isolated applications. It allows MAACC to request capabilities from applications without directly accessing their databases. Combined with SDK integration, tool contract versioning, runtime observability, and strong governance, MAACC can become a scalable enterprise AI agent center for internal applications.
