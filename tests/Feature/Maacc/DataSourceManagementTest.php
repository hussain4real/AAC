...
test('the data sources console page renders with the connection allowlist', function () {
    ...
    $this->withoutVite()
        ...
        ->assertInertia(fn (Assert $page) => $page
            ->component('maacc/data-sources')
            ->where('connections', ['maacc_reporting']));
});
...
test('the shared maacc prop exposes data sources without any secret material', function () {
    ...
    $this->actingAs($owner)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('maacc.dataSources.0.name', 'Reporting Replica')
            ->where('maacc.dataSources.0.credentialManaged', false)
            // The connection name, connection string, and credential never leave the server.
            ->missing('maacc.dataSources.0.connection')
            ->missing('maacc.dataSources.0.vault_secret_id'));
});
