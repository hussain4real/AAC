...
test('the connectors console page renders', function () {
    ...
    $this->withoutVite()
        ...
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page->component('maacc/connectors'));
});
...
test('the connectors page exposes connectors without credential material', function () {
    ...
    $this->actingAs($owner)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('maacc.connectors.0.name', 'Ops MCP')
            ->where('maacc.connectors.0.authConfigured', true)
            ->where('maacc.connectors.0.authType', 'bearer')
            ->where('maacc.connectors.0.toolCount', 0)
            ->where('maacc.connectors.0.capabilities.0.name', 'lookup')
            ->missing('maacc.connectors.0.auth_credential')
            ->missing('maacc.connectors.0.authCredential'));
});
