...
/**
 * Proves the MAACC demo seeder reproduces the Phase 1 console fixture
 * (resources/js/maacc/data.ts) as governed database records.
 */
...
test('credentials store a hashed secret and never the plaintext', function () {
    ...
    expect($credential->secret_hash)->not->toStartWith('maacc_sk_')
        ->and($credential->last_four)->toHaveLength(4)
        ->and(Credential::where('secret_hash', 'like', 'maacc_sk_%')->count())->toBe(0);
});
...
test('the console prop exposes a run\'s real final output', function () {
    ...
    $completed = maaccRun($agent, ['status' => RunStatus::Completed, 'output' => 'Berth utilization is 84%.', 'latency_ms' => 4200]);
    $waiting = maaccRun($agent, ['status' => RunStatus::WaitingForClient, 'output' => null, 'latency_ms' => null]);
