...
test('the evaluation lab page renders', function () {
    ...
    $this->withoutVite()
        ...
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page->component('maacc/evaluations'));
});
...
test('the evaluations page exposes datasets and evaluations', function () {
    ...
    $this->actingAs($owner)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('maacc.evaluationDatasets.0.name', 'Gate')
            ->has('maacc.evaluations', 1)
            ->where('maacc.evaluations.0.label', 'Gate run')
            ->where('maacc.evaluations.0.status', 'passed'));
});
