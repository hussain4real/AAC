 ...
 * the fixtures are generated from MAACC's own logic, a drift here means a
 ...
 */
test('the committed contract fixtures match what MAACC currently produces', function () {
    $path = base_path('packages/sdk-fixtures/contract.json');

    expect(File::exists($path))->toBeTrue('Run `php artisan maacc:sdk-fixtures` to generate the fixtures.')
        ->and(File::get($path))->toBe(
            ContractFixtures::toJson(),
            'SDK contract fixtures are out of date — run `php artisan maacc:sdk-fixtures` and commit.',
        );
});
...
test('the command verifies up-to-date fixtures', function () {
    $this->artisan('maacc:sdk-fixtures', ['--check' => true])->assertSuccessful();
});

test('the command writes the fixtures and they then verify clean', function () {
    $this->artisan('maacc:sdk-fixtures')->assertSuccessful();
    $this->artisan('maacc:sdk-fixtures', ['--check' => true])->assertSuccessful();
});
...
test('the check option fails when the committed fixtures drift', function () {
    ...
    try {
        $this->artisan('maacc:sdk-fixtures', ['--check' => true])->assertFailed();
    } finally {
