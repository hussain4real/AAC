 ...
 * MAACC executes the tool server-side through the real Laravel MCP client (the
...
it('runs an agent whose tool MAACC executes via an MCP connector', function () {
    ...
    $run = $this->client->run(MaaccE2ESeeder::AGENT_SLUG, 'Port status?', new ToolHandlerRegistry, 'cli-connector');

    expect($run->isCompleted())->toBeTrue()
        ->and($run->response)->toBe('Hamad Port is clear.');

    // MAACC executed the tool server-side via the connector and audited it.
...
it('surfaces a connector failure as a controlled run failure to the external app', function () {
    ...
    $run = $this->client->run(MaaccE2ESeeder::AGENT_SLUG, 'Port status?', new ToolHandlerRegistry, 'cli-connector-fail');
