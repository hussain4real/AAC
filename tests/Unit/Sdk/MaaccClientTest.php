...
/**
 * Unit coverage for the SDK client against a scripted transport — no MAACC, no
 * database — pinning the request shapes it sends and how it parses/raises on
 * MAACC's responses.
 */
function sdkClient(FakeTransport $transport): MaaccClient
{
...
it('exchanges the credential for a token via the form grant', function () {
    ...
    expect($request->method)->toBe('POST')
        ->and($request->url)->toBe('https://maacc.test/oauth/token')
...
it('negotiates compatibility with the MAACC sdk endpoint', function () {
    $transport = (new FakeTransport)
        ->push(200, tokenResponse())
        ->push(200, [
            'api_version' => '0.0.1',
    ...
    expect($request->url)->toBe('https://maacc.test/api/v1/sdk')
        ->and($request->headers['X-Maacc-Sdk-Version'])->toBe(MaaccClient::VERSION)
        ->and($request->headers['X-Maacc-Sdk-Language'])->toBe('php');
});
...
it('throws when MAACC pauses for an unregistered tool', function () {
    $transport = (new FakeTransport)
        ->push(200, tokenResponse())
        ->push(201, [
            'run_id' => 'run-1', 'agent_slug' => 'ops', 'status' => 'waiting_for_client',
...
it('raises a typed exception carrying the MAACC error code and status', function () {
    $transport = (new FakeTransport)
        ->push(200, tokenResponse())
    ...
    try {
        ...
        $this->fail('Expected a MaaccApiException.');
