/* ============================================================
   MAACC — Tool contract form (Phase 7)
...
} from '@/actions/App/Http/Controllers/Maacc/ToolContractController';
import {
    Btn,
    Field,
    Input,
...
import type { Tool } from '@/maacc/data';
import {
    EXEC_MODE_OPTIONS,
    FieldError,
    HTTP_METHOD_OPTIONS,
...
export function ToolFormModal({
    ...
    const connectorUuid = tool?.connector
        ...
        : (MAACC.connectors[0]?.uuid ?? '');

    const knowledgeUuid =
        tool?.knowledgeSourceId ?? MAACC.knowledgeSources[0]?.uuid ?? '';

    const dataSourceUuid =
        tool?.dataSourceId ?? MAACC.dataSources[0]?.uuid ?? '';
    ...
    const form = useForm<{
        ...
        application_id: tool?.appId
            ? (MAACC.appById(tool.appId)?.uuid ?? NO_APP)
            : NO_APP,
    ...
    const selectedConnector = MAACC.connectors.find(
    ...
    return (
        <Modal
            ...
            sub="MAACC owns the contract; execution lives where the mode specifies."
            width={640}
            footer={
                <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                            ...
                            <>
                                <Field label="MCP connector" required>
                                    <Select
                                        ...
                                        options={MAACC.connectors.map((c) => ({
                                            value: c.uuid,
                                            label: c.name,
                                        }))}
                                    />
                ...
                {form.data.execution_mode === 'knowledge' && (
                    <div style={configBox}>
                        ...
                        {MAACC.knowledgeSources.length === 0 ? (
                            <div
                                style={{ fontSize: 12, color: 'var(--text-3)' }}
                            >
                            ...
                            <>
                                <Field label="Knowledge source" required>
                                    <Select
                                        ...
                                        options={MAACC.knowledgeSources.map(
                                            (s) => ({
                                                value: s.uuid,
                                                label: s.name,
                                            }),
                            ...
                            <>
                                <Field label="Data source" required>
                                    <Select
                                        ...
                                        options={MAACC.dataSources.map((s) => ({
                                            value: s.uuid,
                                            label: s.name,
                                        }))}
                                    />
