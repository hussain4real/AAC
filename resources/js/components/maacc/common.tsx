...
import type { Scope } from '@/maacc/personas';
...
export function AppHistory({ app }: { app: { id: string } }) {
    ...
    const appRuns = MAACC.runs.filter((r) => r.appId === app.id);
        ...
        >
            {appRuns.map((r) => (
                <Tr key={r.id} onClick={() => go('run', { id: r.id })}>
                    ...
                    <Td>
                        {MAACC.agentById(r.agentId)?.name.replace(' Agent', '')}
                    </Td>
                    <Td mono>{r.caller}</Td>
                    <Td>
 ...
 * authorization is the app's own concern (MAACC's `ctx` is just `{ run, toolCall }`).
 */
...
// MAACC defines the contract; your app owns the execution (keyed by the tool slug).
...
${outList.map((o) => `        '${o}' => $result['${o}'],`).join('\n')}
    ...
    if (lang === 'py') {
        ...
        return `from maacc_sdk import ToolHandlerRegistry
