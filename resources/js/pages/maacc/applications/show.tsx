...
} from '@/actions/App/Http/Controllers/Maacc/ApplicationController';
import {
    revoke as revokeCredential,
    rotate as rotateCredential,
    store as storeCredential,
} from '@/actions/App/Http/Controllers/Maacc/CredentialController';
import {
    AppHistory,
    NoAccess,
    PlaceholderScreen,
} from '@/components/maacc/common';
import {
    APP_STATUS,
    AgentBadge,
    AppMark,
...
import type { MaaccNav } from '@/maacc/nav';
                ...
                <Card pad={false}>
                    ...
                    >
                        {agents.slice(0, 4).map((a) => (
                            ...
                            >
                                <Td strong>{a.name}</Td>
                                <Td mono>{MAACC.llmById(a.llm)?.name}</Td>
                                <Td>
...
function AppAgents({ agents, go }: { agents: Agent[]; go: MaaccNav['go'] }) {
    const MAACC = useMaaccData();
...
function AppTools({ tools, go }: { tools: Tool[]; go: MaaccNav['go'] }) {
    const MAACC = useMaaccData();
            ...
            {tools.map((t) => (
                <Tr key={t.id} onClick={() => go('tool', { id: t.id })}>
                    ...
                    <Td>
                        {t.usedBy
                            .map((u) =>
                                MAACC.agentById(u)?.name.replace(' Agent', ''),
                            )
                            .join(', ')}
                    </Td>
                    <Td>
...
function AppCreds({ app }: { app: Application }) {
    ...
    const envBlock = `MAACC_CLIENT_ID=${activeCred?.clientId ?? 'generate-a-credential'}
MAACC_CLIENT_SECRET=••••••••••••  # shown once on generation
MAACC_ENVIRONMENT=${app.env.toLowerCase()}`;
                ...
                <Card>
                    ...
                    {credentials.length === 0 ? (
                        <EmptyState
                            ...
                            desc="Generate a client ID and secret so this application can authenticate to the MAACC SDK."
                            action={
                                <Btn
                                    variant="primary"
                                    icon="plus"
...
function AppSDK({ app }: { app: Application }) {
    const install = `# Install the MAACC SDK
npm install @qatar-navigation-milaha/sdk`;
    const init = `import { MaaccClient } from "@qatar-navigation-milaha/sdk";
...
const agents = await maacc.listAgents();`;
...
// MAACC defines the contract; this app owns the execution.
...
console.log(res.output);`;
    const steps: {
    ...
    }[] = [
        {
            ...
            desc: 'Add the MAACC SDK to your application dependencies.',
            code: install,
            lang: 'bash',
        },
        {
        ...
        },
        {
            n: 4,
            title: 'Invoke agents',
            desc: 'MAACC pauses for client tools and resumes after your handler responds.',
            code: invoke,
            lang: 'typescript',
        },
    ];
...
export default function Show({ id }: { id: string }) {
    ...
    const appAgents = MAACC.agentsByApp(id);
    const appProjects = MAACC.projectsByApp(id);
    const appTools = MAACC.tools.filter((t) => t.appId === id);
