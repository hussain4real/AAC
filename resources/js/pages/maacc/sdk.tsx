/* ============================================================
   MAACC — SDK Implementation Center
   ============================================================ */
import { Head } from '@inertiajs/react';
import { useState } from 'react';
import { Progress } from '@/components/maacc/charts';
import { Lbl, schemaJson, sdkStub } from '@/components/maacc/common';
import {
    AppMark,
    Badge,
    Btn,
...
function ToolImplPanel({ tool, app, onClose }: ToolImplPanelProps) {
    ...
    const ag = MAACC.agentById(tool.usedBy[0]);
    ...
    const checklist = [
        {
            label: 'Install MAACC SDK in the application',
        ...
        {
            label: 'MAACC re-validates every result against the output schema and rejects a mismatch (invalid_tool_result)',
            done: isImplemented,
        },
        {
            label: 'Report the implementation & sync status to MAACC',
            done: isImplemented,
        },
    ];
...
/**
 * The versioned SDK contract surface: MAACC's API contract version, the supported
...
function SdkVersioningPanel() {
    const MAACC = useMaaccData();
    const { platform, applications, drift } = MAACC.sdkCompatibility;
...
export default function SDKCenter() {
    ...
    const MAACC = useMaaccData();
    const appsInScope = scope.apps.length ? scope.apps : MAACC.apps;
    ...
    const app = MAACC.appById(appId)!;
    const appTools = MAACC.tools.filter(
        (t) => t.appId === appId && t.execMode === 'client',
    );
    const appAgents = MAACC.agentsByApp(appId);
