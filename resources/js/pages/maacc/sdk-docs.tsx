/* ============================================================
   MAACC — SDK Documentation
   A proper, multi-language docs page for the MAACC SDK & runtime
   ...
   docs/MAACC_SDK_Integration_Guide.md.
   ============================================================ */
...
const INSTALL = {
    ...
    php: `composer require maacc/sdk   # PHP >= 8.2, ext-curl, ext-json`,
    py: `# Python: MAACC generates ready-to-paste handler stubs today
# (see "Implementing a handler"); a packaged client is planned.
# Track status in the compatibility matrix below.`,
};

const QUICKSTART_TS = `import { isCompleted, MaaccClient, ToolHandlerRegistry } from '@qatar-navigation-milaha/sdk';
...
// Your data access + permissions live in the handler; MAACC only sees the result.
...
const QUICKSTART_PHP = `use Maacc\\Sdk\\{MaaccClient, MaaccConfig};
...
const TOKEN_EXCHANGE = `POST {MAACC_BASE_URL}/oauth/token
...
const HANDLER_PHP = `use Maacc\\Sdk\\Tools\\{ToolHandler, ToolContext, ToolHandlerRegistry};
...
const HANDLER_PY = `from maacc_sdk import ToolHandlerRegistry
...
const ok = verifyWebhook(
  rawBody,
  req.headers['x-maacc-signature'],
  req.headers['x-maacc-webhook-timestamp'],
...
// …or start async and poll the run yourself:
$queued = $client->startRun('ops-agent', 'Long job', mode: MaaccClient::MODE_ASYNC);
...
// The manifest still shows what MAACC runs for you, tagged with its mode:
...
const VALIDATE_PHP = `use Maacc\\Sdk\\Testing\\ToolTester;
...
const ENV_VARS: {
...
}[] = [
    {
        name: 'MAACC_BASE_URL',
        required: 'yes',
        example: 'https://maacc.test',
        notes: 'Base URL of the MAACC instance (no trailing path).',
    },
    {
        name: 'MAACC_CLIENT_ID',
        required: 'yes',
        example: '9c3f…',
        notes: "The credential's client id.",
    },
    {
        name: 'MAACC_CLIENT_SECRET',
        required: 'yes',
        example: 's3cr3t…',
        notes: 'Shown once on generate/rotate.',
    },
    {
        name: 'MAACC_AGENT_SLUG',
...
const MATRIX: {
    ...
    },
    {
        ...
        status: 'Experimental',
        tone: 'amber',
        notes: 'Stubs are generated by MAACC; a packaged client is coming soon.',
    },
    {
    ...
    },
    {
        ...
        status: 'Supported',
        tone: 'teal',
        notes: 'MAACC executes these server-side; the manifest tags them so the app implements nothing.',
    },
    {
        ...
        status: 'Supported',
        tone: 'teal',
        notes: 'MAACC retrieves cited passages from a governed source server-side; the manifest tags them (execution_mode "knowledge").',
    },
    {
        ...
        status: 'Supported',
        tone: 'teal',
        notes: 'MAACC queries an approved read-only data source server-side under strict policy controls; the manifest tags them (execution_mode "db").',
    },
];
...
const LIFECYCLE: { title: string; body: string }[] = [
    ...
    },
    {
        ...
        body: 'POST /api/v1/tool-implementations with the tool, handler name, version, and fingerprint. MAACC reconciles each to implemented / outdated / incompatible.',
    },
    {
...
const TROUBLESHOOT: { q: string; a: string }[] = [
    ...
    {
        q: 'Nothing appears in the MAACC console',
...
export default function SdkDocs() {
    ...
    const platform = MAACC.sdkCompatibility.platform;
    ...
    useEffect(() => {
        const scroller: HTMLElement | Window =
            document.querySelector<HTMLElement>('main.maacc-scroll') ?? window;
        ...
        <>
            ...
            <div className="route-anim">
                <PageHeader
                    ...
                    sub="Connect any application to MAACC using only the public SDK & runtime APIs — token exchange, manifest sync, implementation reporting, and pause/resume agent runs."
                    actions={
                        <Btn
                            variant="default"
                            icon="chevleft"
                ...
                >
                    ...
                    <div>
                        <DocSection
                            ...
                            title="Overview"
                            icon="info"
                            sub="The MAACC integration model in one paragraph"
                        >
                            <p
                                style={{
                                    fontSize: 13.5,
                            ...
                            >
                                MAACC owns the agent, its prompt, its approved
                            ...
                            >
                                ...
                                <Badge tone="teal">maacc/sdk (PHP)</Badge>
                                <Badge tone="teal">
                        ...
                        <DocSection
                            ...
                            title="Prerequisites"
                            icon="check2"
                            sub="A MAACC operator sets this up once, in the console"
                        >
                            <ol
                                style={{
                                    margin: 0,
                        ...
                        >
                            ...
                            >
                                ...
                                that MAACC posts each lifecycle event to with an
                        ...
                        <DocSection
                            ...
                            icon="layers"
                            sub="MAACC-hosted, remote HTTP, MCP connector, knowledge-retrieval (RAG), and read-only database tools MAACC runs for you"
                        >
                            <p
                                style={{
                                    fontSize: 13,
                            ...
                            >
                                ...
                                allowlisted external endpoint MAACC calls),{' '}
                                <span className="mono">connector</span> (a
                                registered MCP server MAACC connects to),{' '}
                                ...
                                — run inside MAACC. You implement{' '}
                        ...
                        >
                            ...
                            >
                                MAACC generates ready-to-paste stubs like these
                        ...
                        <DocSection
                            ...
                            icon="alert"
                            sub="Every controlled failure is a typed error carrying MAACC's code + HTTP status"
                        >
                            <Table
                                columns={[
                                    { label: 'Code' },
                        ...
                        >
                            ...
                            >
                                {[
                                    ...
                                    {
                                        code: 'packages/maacc-sdk-php/examples',
                                        desc: 'PHP simple + advanced examples.',
                                    },
                                    {
                                        code: 'packages/maacc-sdk-ts/examples',
                                        desc: 'TypeScript simple + advanced examples.',
                                    },
                                    {
                                        code: 'docs/MAACC_SDK_Migration_Guide.md',
