/* ============================================================
   MAACC — MCP Connectors (Phase 6E)
   Register external MCP servers MAACC connects to as a client,
...
} from '@/actions/App/Http/Controllers/Maacc/McpConnectorController';
import {
    Badge,
    Btn,
    Card,
...
import type { MaaccConnector } from '@/types/global';
...
function ConnectorFormModal({
    ...
    return (
        <Modal
            ...
            sub="MAACC connects to this external MCP server as a client to run connector-backed tools."
            width={600}
            footer={
                <>
...
function CapabilitiesPanel({ connector }: { connector: MaaccConnector }) {
    return (
...
export default function Connectors() {
    ...
    const connectors = MAACC.connectors;
    const [modalOpen, setModalOpen] = useState(false);
    const [editing, setEditing] = useState<MaaccConnector | undefined>();
    ...
    const openEdit = (connector: MaaccConnector) => {
    ...
    const discover = (connector: MaaccConnector) => {
        router.post(
    ...
    const remove = (connector: MaaccConnector) => {
        ...
        <>
            ...
            <div className="route-anim">
                <PageHeader
                    ...
                    sub="Register external MCP servers MAACC connects to as a client. Discover their tools, then map them to connector-backed tool contracts — MAACC executes them server-side with the same schema, governance, and audit standards as every other tool."
                    actions={
                ...
                {connectors.length === 0 ? (
                    <Card>
                        <EmptyState
                            ...
                            desc="Register an external MCP server to expose its tools to your agents. MAACC discovers each server’s capabilities and runs the mapped tools server-side."
                            action={
                                <Btn
                                    variant="primary"
                                    icon="plus"
