/* ============================================================
   MAACC — Read-only Data Sources (Phase 8A)
   Register governed read-only data sources (replicas / reporting
   views) a `db` tool may query. MAACC stores only the approved
...
} from '@/actions/App/Http/Controllers/Maacc/DataSourceController';
import {
    Badge,
    Btn,
    Card,
...
import type { MaaccDataSource } from '@/types/global';
...
function DataSourceFormModal({
    ...
    const isEdit = !!source;
    const dbSecrets = MAACC.vaultSecrets.filter((s) => s.kind === 'database');
    ...
    return (
        <Modal
            ...
            sub="A governed read-only surface (replica / reporting view) a db tool may query. MAACC stores only the approved connection name — never a connection string or credential."
            width={640}
            footer={
                <>
...
function DetailPanel({ source }: { source: MaaccDataSource }) {
    return (
...
export default function DataSources() {
    ...
    const sources = MAACC.dataSources;
    const [modalOpen, setModalOpen] = useState(false);
    const [editing, setEditing] = useState<MaaccDataSource | undefined>();
    ...
    const openEdit = (source: MaaccDataSource) => {
        setEditing(source);
        setModalOpen(true);
    };

    const toggleStatus = (source: MaaccDataSource) => {
        router.put(
    ...
    const markRefreshed = (source: MaaccDataSource) => {
        router.post(
    ...
    const remove = (source: MaaccDataSource) => {
                ...
                {sources.length === 0 ? (
                    <Card>
                        <EmptyState
                            ...
                            desc="Register an approved read-only data source so a db tool can run governed reporting queries against it. MAACC never touches application-owned transactional data this way."
                            action={
                                <Btn
                                    variant="primary"
                                    icon="plus"
