...
import {
    ...
    store as storeCase,
} from '@/actions/App/Http/Controllers/Maacc/EvaluationCaseController';
import {
    destroy as destroyEvaluation,
    store as runEvaluation,
} from '@/actions/App/Http/Controllers/Maacc/EvaluationController';
import {
    destroy as destroyDataset,
    store as storeDataset,
    update as updateDataset,
} from '@/actions/App/Http/Controllers/Maacc/EvaluationDatasetController';
import {
    Badge,
    Btn,
    Card,
...
import type { MaaccEvaluation, MaaccEvaluationDataset } from '@/types/global';
...
function DatasetFormModal({
    ...
    dataset?: MaaccEvaluationDataset;
    open: boolean;
    onClose: () => void;
}) {
...
function RunEvaluationModal({
    ...
    const MAACC = useMaaccData();
    const { currentTeam } = usePage().props;
    const agentOptions = MAACC.agents
        .filter((a) => !!a.uuid)
...
function DatasetsTab() {
    ...
    const removeDataset = (dataset: MaaccEvaluationDataset) => {
...
function RunsTab() {
    ...
    const { currentTeam } = usePage().props;
    const teamSlug = currentTeam?.slug ?? '';
    const evaluations = MAACC.evaluations;
    const [selectedId, setSelectedId] = useState<string | null>(null);
    const selected = evaluations.find((e) => e.id === selectedId) ?? null;

    const remove = (evaluation: MaaccEvaluation) => {
...
function ResultsPanel({ evaluation }: { evaluation: MaaccEvaluation }) {
    return (
        <Card style={{ marginTop: 16 }} pad={false}>
            <SectionHeader
...
export default function Evaluations() {
    ...
    const required = MAACC.evaluations.filter((e) => e.isRequired).length;
        ...
        <>
            ...
            <div className="route-anim">
                ...
                >
                    <Stat
                        label="Datasets"
                        value={MAACC.evaluationDatasets.length}
                        icon="flask"
                    />
                    <Stat
                        label="Evaluations run"
                        value={MAACC.evaluations.length}
                        icon="check2"
                        tone="teal"
                    />
                    <Stat
                    ...
                    tabs={[
                        {
                            ...
                            label: 'Datasets',
                            icon: 'flask',
                            count: MAACC.evaluationDatasets.length,
                        },
                        {
                            id: 'runs',
                            label: 'Runs',
                            icon: 'runs',
                            count: MAACC.evaluations.length,
                        },
