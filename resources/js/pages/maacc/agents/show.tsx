...
} from '@/actions/App/Http/Controllers/Maacc/AgentController';
import { VBars, StatCard, Progress } from '@/components/maacc/charts';
import {
    AppHistory,
    NoAccess,
    PlaceholderScreen,
            ...
            {hasToolBrief && (
                <Card>
                    <SectionHeader
                        ...
                        sub="What the model actually receives — your prompt above plus a tool brief MAACC generates automatically from this agent's tools"
                        icon="layers"
                    />
                    <div
                        style={{
                    ...
                    >
                        ...
                        section is generated by MAACC and refreshes when you
...
function AgentAPI({ agent }: { agent: Agent }) {
    const MAACC = useMaaccData();
    const llm = MAACC.llmById(agent.llm);
    const reqBody = `{
        ...
        >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <Card>
                    ...
                    />
                    <CodeBlock
                        code={`Authorization: Bearer <client_token>\nContent-Type: application/json\nX-MAACC-Project: prj_${agent.appId.toLowerCase()}`}
                    />
                </Card>
                <Card pad={false}>
                    <div
 ...
 * `agent_versions` table by {@see \App\Http\Controllers\Maacc\ConsoleController}.
 */
...
function EditAgentModal({
    ...
    const MAACC = useMaaccData();
    const toolOptions = MAACC.tools.map((t) => ({
        value: t.uuid ?? t.id,
        label: t.name,
    }));
    ...
    const form = useForm<{
        ...
        llm_provider_id: MAACC.llmById(agent.llm)?.uuid ?? '',
...
export default function Show({
    ...
    const MAACC = useMaaccData();
    const team = useCurrentTeam();
    const agent = MAACC.agentById(id);
    ...
    const llm = MAACC.llmById(agent.llm);
    const project = MAACC.projectById(agent.projectId);
    const app = MAACC.appById(agent.appId);
    const agentRuns = MAACC.runs.filter((r) => r.agentId === id);
