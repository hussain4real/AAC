...
if (!isSdkCompatible(compatibility)) {
  console.error(
    `Installed SDK v${SDK_VERSION} is ${compatibility.status}; MAACC requires >= ${compatibility.minimumClientVersion}. Upgrade the SDK.`,
  );
  process.exit(1);
}
...
try {
  ...
  if (error instanceof MaaccApiError) {
    console.error(`MAACC error [${error.errorCode}] (HTTP ${error.status}): ${error.message}`);
    process.exit(1);
  }
