...
/** Base class for every error thrown by the MAACC SDK. */
export class MaaccError extends Error {}

/**
 * An HTTP round-trip to MAACC could not complete, or returned an undecodable
 * body — distinct from a controlled error MAACC deliberately returned.
 */
export class TransportError extends MaaccError {
  static fromFetchError(url: string, error: unknown): TransportError {
    ...
    return new TransportError(`Could not reach MAACC at ${url}${detail}${hint}`);
  }
}

/**
 * A controlled error MAACC returned (authentication, unknown agent, oversized
 * payload, quota, ...). The MAACC error code and HTTP status are preserved.
 */
export class MaaccApiError extends MaaccError {
  ...
  static fromResponse(response: HttpResponse): MaaccApiError {
    ...
    const message = typeof payload.message === 'string' ? payload.message : `MAACC returned HTTP ${response.status}.`;

    return new MaaccApiError(code, message, response.status, payload);
  }
}
 ...
 */
export class MissingToolHandlerError extends MaaccError {
...
export class RunNotResolvedError extends MaaccError {
  readonly run: Run;
...
function fetchHint(cause: FetchCause): string {
  ...
  if (cause.code === 'ENOTFOUND') {
    return ' Check MAACC_BASE_URL and make sure the hostname resolves from this process.';
  }
