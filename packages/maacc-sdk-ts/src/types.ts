/**
 * Typed models of MAACC's SDK/runtime response envelopes, plus the small status
...
/**
 * MAACC's verdict on whether the installed SDK client is compatible with a MAACC
 * instance, returned by `MaaccClient.compatibility()`. `status` is one of
...
/** A webhook endpoint registered with MAACC. `secret` is present only on registration. */
...
/** Whether MAACC considers the tool implemented and compatible. */
export function isImplemented(tool: ManifestTool): boolean {
  return tool.implementation.status === 'implemented';
}

/** Whether the installed SDK client can safely talk to this MAACC instance. */
