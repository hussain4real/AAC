import { MaaccApiError, MissingToolHandlerError, RunNotResolvedError, TransportError } from './errors.ts';
...
/** Connection configuration for a MAACC application credential. */
export interface MaaccConfig {
  baseUrl: string;
  clientId: string;
  clientSecret: string;
}
 ...
 * MAACC. It speaks only the documented SDK and runtime contracts — token
 ...
 * PHP-specific.
 */
export class MaaccClient {
  private readonly config: MaaccConfig;
  private readonly transport: Transport;
  private token: CachedToken | null = null;

  constructor(config: MaaccConfig, transport: Transport = fetchTransport) {
    this.config = config;
  ...
  async compatibility(clientVersion?: string): Promise<SdkCompatibility> {
    const response = await this.request({
      ...
      headers: {
        ...
        'X-Maacc-Sdk-Version': clientVersion ?? SDK_VERSION,
        'X-Maacc-Sdk-Language': SDK_LANGUAGE,
      },
    });
  ...
  async run(
    ...
    for (let iteration = 0; isWaiting(run); iteration++) {
      ...
      if (toolCall === null) {
        throw new RunNotResolvedError(run, `The run [${run.runId}] is waiting but MAACC returned no pending tool call.`);
      }
  ...
  async deleteWebhook(id: string): Promise<void> {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw MaaccApiError.fromResponse(response);
    }
  }
  ...
  private authorize(request: HttpRequest, token: string): HttpRequest {
    return {
      ...request,
      headers: {
        'X-Maacc-Sdk-Version': SDK_VERSION,
        'X-Maacc-Sdk-Language': SDK_LANGUAGE,
        ...request.headers,
  ...
  private decode(response: HttpResponse): Record<string, unknown> {
    ...
    try {
      ...
      throw new TransportError(`MAACC returned a non-JSON response (HTTP ${response.status}).`);
    }

    if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
      throw new TransportError(`MAACC returned a non-object JSON response (HTTP ${response.status}).`);
    }
