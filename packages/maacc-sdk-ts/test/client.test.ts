...
import { MaaccClient } from '../src/client.ts';
import { MaaccApiError, MissingToolHandlerError, TransportError } from '../src/errors.ts';
 ...
 * the requests the SDK made — the TypeScript analogue of MAACC's PHP FakeTransport.
 */
...
function client(transport: Transport): MaaccClient {
  return new MaaccClient({ baseUrl: 'https://maacc.test', clientId: 'cid', clientSecret: 'secret' }, transport);
}
...
test('exchanges the credential for a token via the form grant', async () => {
  ...
  assert.equal(requests[0].url, 'https://maacc.test/oauth/token');
...
test('fetch transport surfaces the underlying network failure', async () => {
  ...
  try {
    await assert.rejects(
      () => fetchTransport({ method: 'POST', url: 'https://maacc.test/oauth/token', headers: {} }),
      (error: unknown) => {
...
test('reports registered handlers against the manifest', async () => {
  ...
  const results = await maacc.reportHandlers(await maacc.manifest(), registry, 'typescript');
  ...
  assert.equal(requests[1].headers['X-Maacc-Sdk-Version'], SDK_VERSION);
  assert.equal(requests[1].headers['X-Maacc-Sdk-Language'], 'typescript');
});

test('negotiates compatibility with the MAACC sdk endpoint', async () => {
  const { transport, requests } = fakeTransport([
    TOKEN,
    {
      status: 200,
  ...
  assert.equal(requests[1].url, 'https://maacc.test/api/v1/sdk');
  assert.equal(requests[1].headers['X-Maacc-Sdk-Version'], SDK_VERSION);
});
...
test('throws when MAACC pauses for an unregistered tool', async () => {
...
test('raises a typed error carrying the MAACC error code and status', async () => {
  const { transport } = fakeTransport([
    TOKEN,
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof MaaccApiError && error.errorCode === 'credential_revoked' && error.status === 403,
  );
});
