 ...
 * bundled cURL extension, so the SDK can talk to a live MAACC instance from any
...
final class CurlTransport implements Transport
{
    ...
    public function send(HttpRequest $request): HttpResponse
    {
        ...
        if ($url === '' || $method === '') {
            throw new TransportException('A MAACC request requires a non-empty method and URL.');
        }
        ...
        if ($body === false) {
            ...
            throw new TransportException("Could not reach MAACC at {$url}: {$error}");
        }
