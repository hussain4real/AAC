...
use Maacc\Sdk\Resources\RunEvent;
use Maacc\Sdk\Resources\SdkCompatibility;
...
/**
 * The public entry point for integrating an application with MAACC. It speaks the
 ...
 * — so any consumer (Laravel, plain PHP, or otherwise) drives MAACC the same way.
 *
...
final class MaaccClient
{
    ...
    public function __construct(
        private readonly MaaccConfig $config,
    ...
    /**
     * Ask MAACC whether this installed SDK client is compatible with the server's
    ...
    public function compatibility(?string $clientVersion = null): SdkCompatibility
    {
        $response = $this->request(new HttpRequest('GET', $this->config->url('/api/v1/sdk'), [
            ...
            'X-Maacc-Sdk-Version' => $clientVersion ?? self::VERSION,
            'X-Maacc-Sdk-Language' => self::LANGUAGE,
        ]));
    ...
    /**
     * Report a batch of local tool-handler implementations and return MAACC's
     ...
     * version and schema fingerprint so MAACC can confirm compatibility.
     *
    ...
    public function runAsync(string $agentSlug, string $input, ToolHandlerRegistry $registry, ?string $caller = null, array $options = []): Run
    {
        ...
        for ($iteration = 0; $run->isWaiting(); $iteration++) {
            ...
            if ($toolCall === null) {
                throw new RunNotResolvedException($run, "The run [{$run->runId}] is waiting but MAACC returned no pending tool call.");
            }
    ...
    /**
     * Register a webhook endpoint MAACC will post run lifecycle events to. The
    ...
    public function deleteWebhook(string $id): void
    {
        ...
        if (! $response->successful()) {
            throw MaaccApiException::fromResponse($response);
        }
    }
    ...
    private function authorize(HttpRequest $request, string $token): HttpRequest
    {
        return new HttpRequest($request->method, $request->url, [
            'X-Maacc-Sdk-Version' => self::VERSION,
            'X-Maacc-Sdk-Language' => self::LANGUAGE,
            ...$request->headers,
     ...
     * report to MAACC's SDK Implementation Center.
     */
    ...
    /**
     * Decode a successful response or throw the controlled MAACC error.
     *
     * @return array<string, mixed>
     *
     * @throws MaaccApiException
